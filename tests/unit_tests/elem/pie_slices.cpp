...
namespace cdv::elem
{
    TEST_SUITE("pie slices")
    {
        ...
        {
            auto slices = pie_slices<int>(std::vector<int>(), stdx::identity());
        ...
        {
            auto slices = pie_slices<int>(std::array{3}, stdx::identity());
        ...
        {
            auto slices = pie_slices<int>(std::array{3, 3}, stdx::identity());
            for (const auto& s : slices)
            {
        ...
        {
            auto slices = pie_slices<int>(std::array{3, 3, 6, 12}, stdx::identity());
        ...
        {
            auto slices = pie_slices<int>(std::array{3, 3, 6, 12}, stdx::identity(), {.end_angle = 4_rad});
            auto it = slices.begin();
            CHECK_EQ(it->start_angle, 0_rad);
        ...
        {
            auto slices = pie_slices<int>(std::array{3, 3}, stdx::identity(), {.end_angle = 7_rad});
            auto it = slices.begin();
            CHECK_EQ(it->start_angle, 0_rad);
        ...
        {
            auto slices =
                pie_slices<int>(std::array{3, 3}, stdx::identity(), {.start_angle = 4_rad, .end_angle = 0_rad});
            auto it = slices.begin();
        ...
        {
            auto slices =
                pie_slices<int>(std::array{3, 3}, stdx::identity(), {.end_angle = radians(5.0), .pad_angle = 0.5_rad});
            auto it = slices.begin();
            CHECK_EQ(it->start_angle, 0_rad);
